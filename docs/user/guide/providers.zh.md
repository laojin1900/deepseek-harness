...
通过 OAuth 登录而非使用 API 密钥的提供方——Codex（ChatGPT 订阅）、Claude、GitHub Copilot、Kimi——会出现在提供方列表下方的**订阅登录**卡片中。选择登录方式，打开弹窗给出的页面，按提示输入显示的验证码或在浏览器中完成授权，凭据即保存在本机；登录完成后该提供方的模型即可选用。在你发起登录之前，不会向任何地方发送请求。
...
因此自定义提供方下的视觉模型需要加一行。模型页可以同时设置这两个层级：在**自定义设置**中，**图片输入**开关让该提供方未单独声明的模型接受图片——它写入路由的 `defaultInput`，从未声明过的路由保持关闭；每个模型行的**接受图片**选择器只作用于该模型，其**提供方默认**选项继承路由回退值。同样的设置也写在 `$DSH_HOME/settings.yaml` 中，适合部署时不通过页面固定的场景：
...
- **图片在发送前被拒绝**：该模型未声明图片模态。在自定义提供方里，请把该模型的**接受图片**选择器设为开，或在该模型条目上写 `input: [text, image]`。在 DeepSeek 自身的 chat-completions 路由上，随包发布的 `deepseek-flash`（V4.1-Flash；`deepseek-v4-flash` 与 `deepseek-v4-flash-vision-exp` 是它的服务端别名）已声明图片能力，而 `deepseek-v4-pro` 是纯文本；设置里的 `models` 列表会替换这些默认值，因此未写 `inputModalities` 的条目又会变回纯文本。
