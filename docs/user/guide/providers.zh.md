...
- **图片在发送前被拒绝**：该模型未声明图片模态。在自定义提供方里，请把该模型的**接受图片**选择器设为开，或在该模型条目上写 `input: [text, image]`。在 DeepSeek 自身的 chat-completions 路由上，随包发布的 `deepseek-v4-flash` 已声明图片能力，实验性的 `deepseek-v4-flash-vision-exp` 同样如此；设置里的 `models` 列表会替换这些默认值，因此未写 `inputModalities` 的条目又会变回纯文本。
