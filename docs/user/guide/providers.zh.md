...
- **图片在发送前被拒绝**：该模型未声明图片模态。在自定义提供方里，请把该模型的**接受图片**选择器设为开，或在该模型条目上写 `input: [text, image]`；在 DeepSeek 自身的 chat-completions 路由上，请选择声明了图片能力的目录模型 `deepseek-v4-flash-vision-exp`。
