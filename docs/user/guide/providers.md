...
- **An image is refused before sending** — The model declares no image modality. On a custom provider, set the model's **Accepts images** picker on, or write `input: [text, image]` on the model entry; on DeepSeek's own chat-completions route, select `deepseek-v4-flash-vision-exp`, the catalog model that declares images.
