...
- **An image is refused before sending** — The model declares no image modality. On a custom provider, set the model's **Accepts images** picker on, or write `input: [text, image]` on the model entry. On DeepSeek's own chat-completions route, the shipped `deepseek-v4-flash` declares images, as does the experimental `deepseek-v4-flash-vision-exp`; a settings `models` list replaces those defaults, so an entry written without `inputModalities` is text-only again.
