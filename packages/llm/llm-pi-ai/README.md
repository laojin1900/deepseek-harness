...
The Models settings page exposes both switches: an **Image input** toggle under **Customized settings** writes the route's `defaultInput`, and each model row's **Accepts images** picker writes that entry's `input` (its **Provider default** position leaves the field unset). Both stay off until turned on, matching the text-only default above.
