...
export interface StoredDeepSeekDiscoveryProfile {
  ...
  readonly knownModels: readonly DeepSeekCatalogModel[]
 ...
 * @param storedProfile - resolved endpoint, defaults, known catalog, and stored key.
...
export async function discoverModels(
  ...
  storedProfile?: () => StoredDeepSeekDiscoveryProfile | undefined,
): Promise<readonly LlmDiscoveredModel[]> {
  const stored = storedProfile?.()
  const baseURL = request.baseURL ?? stored?.resolvedBaseURL ?? PUBLIC_BASE_URL
  if (baseURL.length === 0) {
    throw new LlmError(
  ...
  const known = new Map((stored?.knownModels ?? []).map(model => [model.id, model]))
  ...
  for (const curated of stored?.knownModels ?? []) {
  ...
  const defaultContextWindow = stored?.defaultContextWindow
  const defaultMaxTokens = stored?.defaultMaxTokens
  return merged.map((model) => {
    const contextWindow = model.contextWindow ?? defaultContextWindow
