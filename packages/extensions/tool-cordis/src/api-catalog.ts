...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  },
  {
    ...
    methods: [
      ...
      {
        signature: 'registerQuota( settingsNs: string, query: (request: LlmQuotaRequest) => Promise<LlmQuotaResult>, ): () => void',
        description: 'Offer to answer quota/balance queries for the settings namespace this plugin owns. Disposed with the fiber.',
        parameters: [{ name: 'settingsNs', description: 'the namespace whose profiles this quota query serves.' }, { name: 'query', description: 'answers one provider\'s quota; must honor `request.signal`.' }],
        returns: 'the disposer that withdraws the offer.',
      },
      {
        signature: 'async getQuota(settingsNs: string, request: LlmQuotaRequest): Promise<LlmQuotaResult>',
        description: 'Ask one provider\'s quota/balance. The reply is already shaped for display, with an explicit reliability status (`ok` / `unavailable` / `error`).',
        parameters: [{ name: 'settingsNs', description: 'namespace whose registered query serves this provider.' }, { name: 'request', description: 'the provider to ask.' }],
        returns: 'the quota result; throws when no query is registered.',
      },
      {
        signature: '@Remote(\'getQuota\') async remoteGetQuota(provider: string, signal: AbortSignal): Promise<LlmQuotaResult>',
        description: 'Remote adapter for one provider quota/balance query. The Host resolves the owning settings namespace from its configurable-provider directory, so the wire carries only the route the badge asks about.',
        parameters: [{ name: 'provider', description: 'provider route to ask.' }, { name: 'signal', description: 'caller cancellation supplied by the Remote carrier.' }],
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  {
    name: 'LlmQuotaRequest',
    declaration: 'export interface LlmQuotaRequest {\n    provider: string;\n    signal?: AbortSignal;\n}',
  },
  {
    name: 'LlmQuotaResult',
    declaration: 'export interface LlmQuotaResult {\n    status: \'ok\' | \'unavailable\' | \'error\';\n    text: string;\n    detail?: string;\n}',
  },
  {
