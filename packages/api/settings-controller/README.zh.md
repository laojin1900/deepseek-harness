---
description: "settings、凭据与订阅登录配置界面的 Host Remote owner，涵盖脱敏读取、写入、凭据引用、原生文档打开，以及浏览器页面跟随的 OAuth 登录尝试。"
...
`@deepseek-ai/dsh-api-settings-controller` 为浏览器配置界面提供生成的 `ctx.remote.settings`、`ctx.remote.credentials` 与 `ctx.remote.authorization` namespace。它返回脱敏的 settings 与凭据元数据，支持 settings 与凭据写入而不返回密钥值，在 Host 桌面打开由 provider 持有的 settings 或 Agent preset 位置，并承载页面发起、跟随、应答与取消的订阅登录。provider 或授权 seam 缺失时，namespace 仍会注册，并返回可操作的配置错误。
...
请把本包作为 Loader entry 挂载到提供浏览器配置的 profile 中。本 entry 不依赖 provider 是否存在而注册全部三个 namespace，因此缺少 provider 会在调用时产生具名配置错误；未挂载登录 seam 时，authorization namespace 会给出自己的可操作错误。它生成的 descriptor 进入严格 Typert registry，而 settings 与凭据 Definition 仍是普通 Cordis Service，自身不承担任何 wire 义务。
...
`authorization.list()` 报告已挂载的 flow owner 注册的全部登录流程，以及其凭据键对应的 provider 路由。`authorization.begin(key, method)` 发起一次尝试并返回其 id；`authorization.poll(attemptId, since)` 返回该尝试的状态、调用方游标之后的通知，以及它正在等待的提问；`authorization.answer(attemptId, promptId, value)` 只应答该提问，编号已不匹配的应答会被拒绝；`authorization.cancel(attemptId)` 撤回该尝试。尝试保存在 Host 中，每个键同时只有一个，数量有界并会自动清理；通知与提问视图都不携带凭据值，流程自身的 `AbortSignal` 也绝不跨越 wire。
